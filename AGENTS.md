...
- **Shared WebSocket (`@/services/socket`)**: `getSocket()`/`connectSocket()`/`disconnectSocket()` manage one lazily-created, auto-reconnecting `socket.io-client` instance shared across the whole app — don't call `io(...)` directly in a component. Connect/disconnect lifecycle is tied to auth state (see `NotificationContext`'s effect), not to any one component's mount/unmount. A future feature needing realtime data (e.g. live scrim chat) should listen on the same shared socket rather than opening a second connection.
