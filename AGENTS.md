...
- **Global Contexts**:
  ...
  - `useNotifications()`: Receives live push events over `@/services/socket` (`notification:new/updated/all-read/cleared`) for the logged-in user, with `GET /notifications` used only for initial hydration and reconnect catch-up. `NotificationBell`/`FloatingNotificationToast` render whatever it returns and stay hidden entirely when logged out. Notifications are created server-side (see `collegium-server/AGENTS.md` §6) — never invent a client-side notification by writing to this context's state directly.
- **Shared WebSocket (`@/services/socket`)**: `getSocket()`/`connectSocket()`/`disconnectSocket()` manage one lazily-created, auto-reconnecting `socket.io-client` instance shared across the whole app — don't call `io(...)` directly in a component. Connect/disconnect lifecycle is tied to auth state (see `NotificationContext`'s effect), not to any one component's mount/unmount. `ScrimWarRoomModal` is the reference pattern for a feature-scoped room on this same shared socket: `getSocket().emit("scrim:join"/"scrim:leave", scrimId)` on open/close, `.on("scrim:message", ...)` for live messages, one REST call (`getScrimChat`) for history hydration on open. A future realtime feature should follow the same shape rather than opening a second connection.
